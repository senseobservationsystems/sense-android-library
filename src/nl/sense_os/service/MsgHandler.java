...
public class MsgHandler {
	...
	public class MultiServerThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				final BufferedReader in = new BufferedReader(new InputStreamReader(this.socket
						.getInputStream()));
	...
	public MsgHandler(Context context) {
		...
		this.url = prefs.getString(PREF_KEY_URL, "http://demo.almende.com/commonSense/sp_state_add.php");
	...
	*/
	...
	{
		String url = SenseSettings.URL_QUIZ_ADD_QUESTION+"?question="+URLEncoder.encode(question);
		final SharedPreferences prefs 	= context.getSharedPreferences(PRIVATE_PREFS, android.content.Context.MODE_PRIVATE);
		String cookie = prefs.getString(SenseSettings.PREF_LOGIN_COOKIE, "");			
		new Thread(new SendMessageThread(url, cookie)).start();		
	}
