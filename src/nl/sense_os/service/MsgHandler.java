...
public class MsgHandler extends Service {
    ...
    private class SendSingleDataThread implements Runnable {
        String cookie;
        String url;

        public SendSingleDataThread(String _url, String Cookie) {
            url = _url;
            cookie = Cookie;
        }
    ...
    private void handleNewMsgIntent(Intent intent) {
        ...
        if (isOnline()) {
            ...
            if (prefs.getString(SenseSettings.PREF_SYNC_RATE, "0").equals("-2")) {
                ...
                sendSingleData(name, value, type, device);
            } else {
    ...
    private boolean sendDataFromBuffer() {
        ...
        if (this.bufferCount > 0) {
            ...
            final boolean success = sendBatchData(json);
    ...
    public void sendSingleData(String sensorName, String sensorValue, String dataType,
            String deviceType) {
        ...
        if (nrOfSendMessageThreads < MAX_NR_OF_SEND_MSG_THREADS) {
            ++nrOfSendMessageThreads;
            new Thread(new SendSingleDataThread(url, cookie)).start();
        }
    }
}
