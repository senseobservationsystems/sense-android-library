...
public class SenseService extends Service {
	...
	private boolean login(String email, String pass) {		
		try {			
			...
			HashMap<String, String> response = MsgHandler.sendJson(url, user, "POST", "");
			if (response == null) 
				return false;
			...
			{
				// incorrect login
				Log.e(TAG, "CommonSense login incorrect! response code:" + response.get("http response code"));				
	...
	private boolean register(String email, String pass) {
		try {			
			...
			HashMap<String, String> response = MsgHandler.sendJson(url, newUser, "POST", "");
			if (response == null) {
				return false;
			}
	 ...
	 * A phone as device will be registered while registering a sensor
	 */
	...
	private void getRegisteredSensors()
	{
		try{
			...
			JSONObject response = MsgHandler.getJSONObject(uri, cookie);
			if(response != null)
			{
	...
	public boolean senseServiceRegister() {
		...
		if ((email.length() > 0) && (pass.length() > 0) && register(email, pass) && senseServiceLogin()) {
