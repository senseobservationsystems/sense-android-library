...
public class SenseService extends Service {
	...
	private class SenseServiceStub extends ISenseService.Stub {
		...
		@Override
		public int changeLogin(String username, String password)
		...
		public boolean getPrefBool(String key, boolean defValue)
			...
			if (key.equals(Status.AMBIENCE) || key.equals(Status.DEV_PROX)
					...
					|| key.equals(Status.LOCATION) || key.equals(Status.MAIN)
					|| key.equals(Status.MOTION)
					|| key.equals(Status.PHONESTATE)
					|| key.equals(Status.POPQUIZ)
					|| key.equals(Status.AUTOSTART)) {
				prefs = getSharedPreferences(SensePrefs.STATUS_PREFS,
						MODE_PRIVATE);
			} else {
				prefs = getSharedPreferences(SensePrefs.MAIN_PREFS,
						MODE_PRIVATE);
			}
		...
		public float getPrefFloat(String key, float defValue)
			...
			SharedPreferences prefs = getSharedPreferences(
					SensePrefs.MAIN_PREFS, MODE_PRIVATE);
			try {
		...
		public long getPrefLong(String key, long defValue)
			...
			if (key.equals(Auth.SENSOR_LIST_TIME)) {
				prefs = getSharedPreferences(SensePrefs.AUTH_PREFS,
						MODE_PRIVATE);
			} else {
				prefs = getSharedPreferences(SensePrefs.MAIN_PREFS,
						MODE_PRIVATE);
		...
		public String getPrefString(String key, String defValue)
			...
			if (key.equals(Auth.LOGIN_COOKIE) || key.equals(Auth.LOGIN_PASS)
					...
					|| key.equals(Auth.SENSOR_LIST)
					|| key.equals(Auth.DEVICE_ID)
					|| key.equals(Auth.PHONE_IMEI)
		...
		public int register(String username, String password, String name,
				...
				throws RemoteException {
			return SenseService.this.register(username, password, name,
					surname, email, mobile);
		}
	...
	private HandlerThread ambienceThread, motionThread, deviceProxThread,
			extSensorsThread, locationThread, phoneStateThread;

	/**
	 * Changes login of the Sense service. Removes "private" data of the
	 * previous user from the preferences. Can be called by Activities that are
	...
	private int changeLogin(String username, String password) {
		...
		Editor authEditor = getSharedPreferences(SensePrefs.AUTH_PREFS,
				MODE_PRIVATE).edit();
	...
	/**
	 * Checks if the installed Sense Platform application has an update
	 * available, alerting the user via a Toast message.
	 */
	private void checkVersion() {
		try {
			...
			Map<String, String> response = SenseApi.request(this, url, null,
					null);
			JSONObject content = new JSONObject(response.get("content"));

			if (content.getString("message").length() > 0) {
				Log.i(TAG,
						"Newer Sense Platform version available: "
								+ content.toString());
			...
			if (null != e.getMessage()) {
				Log.e(TAG, "Failed to get Sense Platform version! Message: "
						+ e.getMessage());
			} else {
	...
	private int login() {
		...
		SharedPreferences mainPrefs = getSharedPreferences(
				SensePrefs.MAIN_PREFS, MODE_PRIVATE);
		...
		SharedPreferences authPrefs = getSharedPreferences(
				SensePrefs.AUTH_PREFS, MODE_PRIVATE);
		...
		if ((username != null) && (pass != null)) {
			try {
				...
				Log.w(TAG,
						"Exception during login! " + e + ": '" + e.getMessage()
								+ "'");
				// handle result below
			}
		} else {
	...
	/**
	 * Does nothing except poop out a log message. The service is really started
	 * in onStart, otherwise it would also start when an activity binds to it.
	 */
	@Override
	...
	/**
	 * Performs tasks after successful login: update status bar notification;
	 * start transmitting collected sensor data.
	 */
	private void onLogIn() {
		...
		// store this login
		SharedPreferences prefs = getSharedPreferences(SensePrefs.MAIN_PREFS,
				MODE_PRIVATE);
		prefs.edit()
				.putLong(SensePrefs.Main.LAST_LOGGED_IN,
	...
	/**
	 * Performs cleanup tasks when the service is logged out: updates the status
	 * bar notification; stops the periodic alarms for data transmission.
	 */
	...
	/**
	 * Starts the Sense service. Tries to log in and start sensing; starts
	 * listening for network connectivity broadcasts.
	 * 
	 * @param intent
	 *            The Intent supplied to {@link Activity#startService(Intent)}.
	 *            This may be null if the service is being restarted after its
	 *            process has gone away.
	 * @param flags
	 *            Additional data about this start request. Currently either 0,
	 *            {@link Service#START_FLAG_REDELIVERY} , or
	 *            {@link Service#START_FLAG_RETRY}.
	 * @param startId
	...
	public int onStartCommand(final Intent intent, int flags, int startId) {
		...
		new Handler(startThread.getLooper()) {
			...
			public void handleMessage(Message msg) {
				...
				boolean mainStatus = getSharedPreferences(
						SensePrefs.STATUS_PREFS, MODE_PRIVATE).getBoolean(
						Status.MAIN, true);
				if (false == mainStatus) {
					Log.w(TAG,
							"Sense service was started when the main status is not set!");
					...
					if (false == state.isForeground()) {
						Notification n = ServiceStateHelper.getInstance(
								SenseService.this).getStateNotification();
					...
					relogin |= (null == intent); // intent is null when Service
													// was killed
					relogin |= (null != intent)
							&& intent.getBooleanExtra(EXTRA_RELOGIN, false);
					if (relogin) {
						login();
					} else {
						checkVersion();
	...
	/**
	 * Tries to register a new user using the username and password from the
	 * private preferences and updates the {@link #isLoggedIn} status
	 * accordingly. Can also be called from Activities that are bound to the
	 * service.
	 * 
	 * @param mobile
	 * @param email
	 ...
	 *         exists, and -1 for any other errors.
	 */
	private int register(String username, String password, String name,
			String surname, String email, String mobile) {
	...
	private void showToast(final String message) {
		toastHandler.post(new Runnable() {
			@Override
			public void run() {
				Toast.makeText(SenseService.this, message, Toast.LENGTH_LONG)
						.show();
			}
		});
	}
	...
	/**
	 * Toggles the individual sensor modules according to the status that was
	...
	private void startSensorModules() {
		...
		if (statusPrefs.getBoolean(Status.MAIN, true)) {
			...
			toggleExternalSensors(statusPrefs
					.getBoolean(Status.EXTERNAL, false));
	...
	private synchronized void verifySensorIds() {
		...
		if (new PhoneSensorRegistrator(SenseService.this).verifySensorIds(null,
				null)) {
		...
		if (active != state.isAmbienceActive()) {
			...
			if (true == active) {
				...
				if ((ambienceThread != null) && ambienceThread.isAlive()) {
					Log.w(TAG,
							"Ambience thread is already present! Quitting the thread...");
					ambienceThread.getLooper().quit();
					ambienceThread = null;
				}

				// get sample rate from preferences
				final SharedPreferences mainPrefs = getSharedPreferences(
						SensePrefs.MAIN_PREFS, MODE_PRIVATE);
				...
				final boolean agostinoMode = mainPrefs.getBoolean(
						"agostino_mode", false);
				if (agostinoMode) {
				...
				new Handler(ambienceThread.getLooper()).post(new Runnable() {
					...
					public void run() {
						...
						if (mainPrefs.getBoolean(Ambience.CAMERA_LIGHT, true)) {
							cameraLightSensor = new CameraLightSensor(
									SenseService.this);
		...
		if (active != state.isDevProxActive()) {
			...
			if (true == active) {
				...
				if ((deviceProxThread != null) && deviceProxThread.isAlive()) {
					Log.w(TAG,
							"Device proximity thread is already present! Quitting the thread...");
		...
		if (active != state.isExternalActive()) {
			...
			if (true == active) {
				...
				if ((extSensorsThread != null) && extSensorsThread.isAlive()) {
					Log.w(TAG,
							"Ext. sensors thread is already present! Quitting the thread...");
				...
				new Handler(extSensorsThread.getLooper()).post(new Runnable() {
					...
					public void run() {
						...
						if (mainPrefs.getBoolean(
								External.ZephyrBioHarness.MAIN, false)) {
							es_bioHarness = new ZephyrBioHarness(
									SenseService.this);
							es_bioHarness.startBioHarness(finalInterval);
						}
						if (mainPrefs
								.getBoolean(External.ZephyrHxM.MAIN, false)) {
							es_HxM = new ZephyrHxM(SenseService.this);
							es_HxM.startHxM(finalInterval);
						}
						if (mainPrefs.getBoolean(External.OBD2Sensor.MAIN,
								false)) {
							Log.d(TAG, "Attempting to start OBD2");
							es_obd2sensor = new NewOBD2DeviceConnector(
									SenseService.this, finalInterval);
							es_obd2sensor.run();
						} else {
		...
		if (active != state.isLocationActive()) {
			...
			if (true == active) {
				...
				if ((locationThread != null) && locationThread.isAlive()) {
					Log.w(TAG,
							"Location thread is already present! Quitting the thread...");
		...
		if (active != state.isMotionActive()) {
			...
			if (true == active) {
				...
				if (motionSensor != null) {
					Log.w(TAG,
							"Motion sensor is already present! Stopping the sensor...");
				...
				if ((motionThread != null) && motionThread.isAlive()) {
					Log.w(TAG,
							"Motion thread is already present! Quitting the thread...");
						...
						try {
							if (mainPrefs.getBoolean(PhoneState.BATTERY, true)) {
								batterySensor = new BatterySensor(
										SenseService.this);
								batterySensor
										.startBatterySensing(finalInterval);
							}
							if (mainPrefs.getBoolean(
									PhoneState.SCREEN_ACTIVITY, true)) {
								phoneActivitySensor = new PhoneActivitySensor(
										SenseService.this);
								phoneActivitySensor
										.startPhoneActivitySensing(finalInterval);
							}
							if (mainPrefs
									.getBoolean(PhoneState.PROXIMITY, true)) {
								proximitySensor = new ProximitySensor(
										SenseService.this);
								proximitySensor
										.startProximitySensing(finalInterval);
							}
							phoneStateListener = new SensePhoneState(
									SenseService.this);
