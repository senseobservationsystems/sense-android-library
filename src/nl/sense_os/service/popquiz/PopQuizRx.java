...
public class PopQuizRx extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if ((entryId < 0) || (quizId < 0)) {
            Log.d(TAG, "Something is wrong, canceling alarms... Entry ID: " + entryId + ", quiz ID: " + quizId);
            
            // cancel the alarms            
            mgr.cancelEntry();
        } else {
            Log.d(TAG, "Received pop quiz alarm..."  + " Quiz " + quizId + ", time " + new SimpleDateFormat("HH:mm:ss").format(new Date(entryId)));
            ...
            final boolean silentMode = prefs.getBoolean(SenseSettings.PREF_QUIZ_SILENT_MODE, false);
