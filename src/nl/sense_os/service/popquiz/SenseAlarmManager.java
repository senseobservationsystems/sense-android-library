...
public class SenseAlarmManager {
    ...
    public void createSyncAlarm() {
        ...
        SharedPreferences mainPrefs = this.ctx.getSharedPreferences(SensePrefs.MAIN_PREFS,
                Context.MODE_PRIVATE);
        long lastUpdate = mainPrefs.getLong(Main.Quiz.SYNC_TIME, 0);
    ...
    private long getNextQuarterHour() {
        final SharedPreferences mainPrefs = ctx.getSharedPreferences(SensePrefs.MAIN_PREFS,
                Context.MODE_PRIVATE);
        final int rate = Integer.parseInt(mainPrefs.getString(Main.Quiz.RATE, "0"));
        long period = 0;
        switch (rate) {
        case -1:
            // often (5 mins)
