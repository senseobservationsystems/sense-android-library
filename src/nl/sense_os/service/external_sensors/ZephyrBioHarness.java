...
public class ZephyrBioHarness {
	...
	private class ProcessZephyrBioHarnessMessage
	{
		...
		{	
			try
			{
				...
				if(buffer[0] == 0x02 && buffer[1] == 0x20 && buffer[2] == 53)
				{
					...
					if(prefs.getBoolean(SenseSettings.PREF_BIOHARNESS_HEART_RATE, true))
					{
						...
						heartRateIntent.putExtra(MsgHandler.KEY_SENSOR_DEVICE, "BioHarness "+deviceName);
						heartRateIntent.putExtra(MsgHandler.KEY_VALUE, heartRate.intValue());
						heartRateIntent.putExtra(MsgHandler.KEY_DATA_TYPE, SenseSettings.SENSOR_DATA_TYPE_INT);
					...
					if(prefs.getBoolean(SenseSettings.PREF_BIOHARNESS_RESP, true))
					{
						...
						respirationRateIntent.putExtra(MsgHandler.KEY_SENSOR_DEVICE, "BioHarness "+deviceName);
						respirationRateIntent.putExtra(MsgHandler.KEY_VALUE, respirationRateF);
						respirationRateIntent.putExtra(MsgHandler.KEY_DATA_TYPE, SenseSettings.SENSOR_DATA_TYPE_FLOAT);
						respirationRateIntent.putExtra(MsgHandler.KEY_TIMESTAMP, System.currentTimeMillis());
					...
					if(prefs.getBoolean(SenseSettings.PREF_BIOHARNESS_TEMP, true))
					{
						...
						skinTemperatureIntent.putExtra(MsgHandler.KEY_SENSOR_DEVICE, "BioHarness "+deviceName);
						skinTemperatureIntent.putExtra(MsgHandler.KEY_VALUE, skinTemperatureF);
						skinTemperatureIntent.putExtra(MsgHandler.KEY_DATA_TYPE, SenseSettings.SENSOR_DATA_TYPE_FLOAT);
						skinTemperatureIntent.putExtra(MsgHandler.KEY_TIMESTAMP, System.currentTimeMillis());
					...
					{						
						...
						batteryIntent.putExtra(MsgHandler.KEY_DATA_TYPE, SenseSettings.SENSOR_DATA_TYPE_INT);
					...
					if(prefs.getBoolean(SenseSettings.PREF_BIOHARNESS_BLOOD_PRESSURE, true))
					{
						...
						batteryIntent.putExtra(MsgHandler.KEY_DATA_TYPE, SenseSettings.SENSOR_DATA_TYPE_FLOAT);
						batteryIntent.putExtra(MsgHandler.KEY_TIMESTAMP, System.currentTimeMillis());
						context.startService(batteryIntent);
					}
					...
					{						
						...
						batteryIntent.putExtra(MsgHandler.KEY_DATA_TYPE, SenseSettings.SENSOR_DATA_TYPE_BOOL);
						batteryIntent.putExtra(MsgHandler.KEY_TIMESTAMP, System.currentTimeMillis());
						context.startService(batteryIntent);
					}
				}
			...
			{
				Log.e(TAG, "Error in ProcessZephyrBioHanessMessage:" + e.getMessage());
			}
		}
	}
	...
	private class UpdateThread extends Thread {	   
			...
			if (bioHarnessEnabled) {
				try {						
					...
					{						
						...
						if(bytes > 0)
							processZBHMessage.processMessage(buffer);
						buffer = null;
					}						
					// update every second
					sendConnectionAlive();
				...
				} catch (Exception e) 
				{
					...
					e.printStackTrace();		
					// re-connect
					connectHandler.postDelayed(bioHarnessConnectThread2_1 = new BioHarnessConnectThread2_1(), 1000);
				}					
			}
			else
				cancel();
		...
		public boolean setEnableGeneralData(boolean enable)
		{
			try{
				...
				Log.d(TAG, "Enable received:"+ Integer.toString(buffer[0] &0xFF,16)+ "," + Integer.toString(buffer[1] &0xFF,16) + "," +  Integer.toString(buffer[2] &0xFF,16));
			...
			public void scanCompleted(ArrayList<String> devices) {
				...
				try {
					if (devices.size() != 0)
					{
						...
						{  
							RemoteBluetoothDevice rbtDevice = btDevice.getRemoteBluetoothDevice(devices.get(x));
							if(rbtDevice.isPaired() && rbtDevice.getName().startsWith("BH ZBH") &&  rbtDevice.getAddress().startsWith("00:07:80"))
							{
								btSocket1_6 = rbtDevice.openSocket(1);
								processZBHMessage = new ProcessZephyrBioHarnessMessage(rbtDevice.getName());
								updateHandler.post(updateThread = new UpdateThread());
								foundDevice = true;
								break;
							}
						...
						//							{   								
						//								RemoteBluetoothDevice rbtDevice = btDevice.getRemoteBluetoothDevice(devices.get(x));								
						//								if(rbtDevice.getName().startsWith("BH ZBH") &&  rbtDevice.getAddress().startsWith("00:07:80"))
						//								{
						//									Log.d(TAG, "found device, pairing...");		
						//									rbtDevice.setPin("1234");									
						//									rbtDevice.pair();
						//									it.gerdavax.android.bluetooth.BluetoothSocket btSocket = rbtDevice.openSocket(1);
	...
	private class BioHarnessConnectThread2_1 implements Runnable {
		...
		public void run() {
			if (bioHarnessEnabled) {
				...
				{
					btAdapter.enable();
					Log.d(TAG, "Bluetooth enabled for the BioHarness, waiting 1 sec");
					int cnt = 0;
					try {
				...
				if (pairedDevices.size() > 0) {
					...
					for (BluetoothDevice device : pairedDevices) {
						...
						if(device.getName().startsWith("BH ZBH") &&  device.getAddress().startsWith("00:07:80"))						
						{
							...
							try {
								...
								btSocket2_1.connect();		
								processZBHMessage = new ProcessZephyrBioHarnessMessage(btSocket2_1.getRemoteDevice().getName());
								updateHandler.post(updateThread = new UpdateThread());
								foundDevice = true;
							} catch (Exception e) 
							{
								Log.d(TAG, "Error in connecting to BioHarness device:" + e.getMessage());
							}
						}
					}
				}
				if(!foundDevice)
				{
					...
					connectHandler.postDelayed(bioHarnessConnectThread2_1 = new BioHarnessConnectThread2_1(), 10000);
				}
			} else {
				stop();
			}
		...
		public void stop()
		{
			...
			{
				Log.e(TAG,"Error in stopping bioHarness ervice" + e.getMessage());
			}
		}
		...
		Thread t = new Thread() {
			public void run() {
				...
				if (Build.VERSION.SDK_INT < Build.VERSION_CODES.ECLAIR)
					...
					connectHandler.post(bioHarnessConnectThread2_1 = new BioHarnessConnectThread2_1());
				}
			}
		};
		...
		try {
			if (Build.VERSION.SDK_INT < Build.VERSION_CODES.ECLAIR)				
				if(bioHarnessConnectThread1_6 != null)
				{
					...
					if(bioHarnessConnectThread2_1 != null)
					{
