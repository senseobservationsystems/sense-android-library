			...
			try
			{
				...
				{				
					...
					if(prefs.getBoolean(SenseSettings.PREF_HXM_SPEED, true))
					{
						...
						heartRateIntent.putExtra(MsgHandler.KEY_DATA_TYPE, SenseSettings.SENSOR_DATA_TYPE_FLOAT);
						heartRateIntent.putExtra(MsgHandler.KEY_TIMESTAMP, System.currentTimeMillis());
						context.startService(heartRateIntent);
					}	
					// send distance
	...
	private class HxMConnectThread1_6 implements Runnable {
		...
		private class BluetoothDeviceListener implements LocalBluetoothDeviceListener {
			...
			public void scanCompleted(ArrayList<String> devices) {
				...
				try {
					if (devices.size() != 0)
					{
						...
						{  
							...
							if(rbtDevice.isPaired() && rbtDevice.getName().startsWith("HXM") &&  rbtDevice.getAddress().startsWith("00:07:80"))
							{
								btSocket1_6 = rbtDevice.openSocket(1);
								processZHxMMessage = new ProcessZephyrHxMMessage(btSocket1_6.getRemoteBluetoothDevice().getName());				
				...
				if(!foundDevice)				
					connectHandler.postDelayed(hxmConnectThread1_6 = new HxMConnectThread1_6(), 10000);
			}
	...
	private class HxMConnectThread2_1 implements Runnable {
		...
		public void run() {
			if (hxmEnabled) {
				...
				{
					btAdapter.enable();
					Log.d(TAG, "Bluetooth enabled for the HxM, waiting 1 sec");
					int cnt = 0;
					try {
				...
				if (pairedDevices.size() > 0) {
					...
					for (BluetoothDevice device : pairedDevices) {
						...
						if(device.getName().startsWith("HXM") &&  device.getAddress().startsWith("00:07:80"))						
						{
							...
							try {
								...
								btSocket2_1.connect();								
								processZHxMMessage = new ProcessZephyrHxMMessage(btSocket2_1.getRemoteDevice().getName());
								updateHandler.post(updateThread = new UpdateThread());
								foundDevice = true;
							} catch (Exception e) 
							{
								Log.d(TAG, "Error in connecting to HxM device:" + e.getMessage());
							}
						}
					}
				}
				if(!foundDevice)
				{
					...
					connectHandler.postDelayed(hxmConnectThread2_1 = new HxMConnectThread2_1(), 10000);
				}
			} else {
				stop();
			}
