...
public class SenseApi {
	 ...
	 */
	public static JSONArray getAllSensors(Context context) throws IOException,
			JSONException {
		...
		try {
			String cachedSensors = sAuthPrefs.getString(
					Auth.SENSOR_LIST_COMPLETE, null);
			long cacheTime = sAuthPrefs.getLong(Auth.SENSOR_LIST_COMPLETE_TIME,
					0);
		...
		while (!done) {
			...
			if (null == sMainPrefs) {
				sMainPrefs = context.getSharedPreferences(
			...
			String url = devMode ? SenseUrls.ALL_SENSORS_DEV
					: SenseUrls.ALL_SENSORS;
			url += "&page=" + page;
			Map<String, String> response = SenseApi.request(context, url, null,
					cookie);
		...
		authEditor.putLong(Auth.SENSOR_LIST_COMPLETE_TIME,
	 ...
	 * @throws JSONException
	 */
	public static List<String> getConnectedSensors(Context context,
			String sensorId) throws IOException, JSONException {
		...
		String url = devMode ? SenseUrls.CONNECTED_SENSORS_DEV
				: SenseUrls.CONNECTED_SENSORS;
		...
		if (!"200".equals(responseCode)) {
			Log.w(TAG,
					"Failed to get list of connected sensors! Response code: "
							+ responseCode);
			throw new IOException("Incorrect response from CommonSense: "
					+ responseCode);
		}
	...
	public static String getDeviceConfiguration(Context context)
		...
		String cookie = sAuthPrefs.getString(Auth.LOGIN_COOKIE, null);
		boolean devMode = sMainPrefs.getBoolean(Advanced.DEV_MODE, false);
		String url = devMode ? SenseUrls.DEVICE_CONFIGURATION_DEV
				: SenseUrls.DEVICE_CONFIGURATION;
		...
		String responseCode = response.get(RESPONSE_CODE);
		if (!"200".equals(responseCode)) {
			Log.w(TAG, "Failed to get device configuration! Response code: "
					+ responseCode);
			throw new IOException("Incorrect response from CommonSense: "
					+ responseCode);
		}
	 ...
	 */
	public static String getDeviceConfiguration(Context context,
			String configuration_id) throws IOException, JSONException {
		...
		String url = devMode ? SenseUrls.CONFIGURATION_DEV
				: SenseUrls.CONFIGURATION;
		...
		String responseCode = response.get(RESPONSE_CODE);
		if (!"200".equals(responseCode)) {
			Log.w(TAG, "Failed to get Requirement! Response code: "
					+ responseCode);
			throw new IOException("Incorrect response from CommonSense: "
					+ responseCode);
		}
	...
	private static List<JSONObject> getMatchingSensors(Context context,
			String name, String description, String dataType)
			throws IOException, JSONException {
		...
		for (int i = 0; i < sensors.length(); i++) {
			...
			if (sensor.getString("name").equalsIgnoreCase(name)
					&& ((null == description) || sensor
							.getString("device_type").equalsIgnoreCase(
									description))
					&& ((null == dataType) || sensor.getString("data_type")
							.equalsIgnoreCase(dataType))) {
				result.add(sensor);

			} else if (name.equals(SensorNames.ACCELEROMETER)
					|| name.equals(SensorNames.ORIENT)
					|| name.equals(SensorNames.GYRO)
					|| name.equals(SensorNames.LIN_ACCELERATION)
				...
				// special case to take care of changed motion sensor
				// descriptions since Gingerbread
	...
	/**
	 * Gets the sensor ID at CommonSense , which can be used to modify the
	 ...
	 * @param description
	 *            Sensor description (previously 'device_type'), to match with
	 *            registered sensors.
	 * @param dataType
	 *            Sensor data type, to match with registered sensors.
	 * @param deviceUuid
	 *            (Optional) UUID of the device that should hold the sensor.
	 * @return String with the sensor's ID, or null if the sensor does not exist
	 *         at CommonSense (yet).
	 * @throws IOException
	 ...
	 */
	public static String getSensorId(Context context, String name,
			String description, String dataType, String deviceUuid)
			throws IOException, JSONException {

		// get list of sensors with matching description
		List<JSONObject> sensors = getMatchingSensors(context, name,
		...
		for (JSONObject sensor : sensors) {
			if (null != deviceUuid && !NO_DEVICE_UUID.equals(deviceUuid)) {
				...
				if (null != device
						&& deviceUuid.equals(device.optString("uuid"))) {
					id = sensor.getString("id");
					break;
				}
			} else {
				// we do not care about the device, just accept the first match
				// we find
				id = sensor.getString("id");
				break;
			}
		...
		if (null == id) {
			Log.d(TAG, "missing id for " + name + " (" + description + ") @ "
					+ deviceUuid);
		}
		return id;
	}
	 ...
	 * @return String with the sensor's URL, or null if sensor does not have an
	 *         ID (yet)
	 * @throws JSONException
	 ...
	 */
	public static String getSensorUrl(Context context, String name,
			String description, String dataType, String deviceUuid)
			throws IOException, JSONException {

		String id = getSensorId(context, name, description, dataType,
				deviceUuid);

		if (id == null) {
			Log.w(TAG, "Failed to get URL for sensor '" + name
		...
		if (dataType.equals(SenseDataTypes.FILE)) {
			String url = devMode ? SenseUrls.SENSOR_FILE_DEV
					: SenseUrls.SENSOR_FILE;
			return url.replaceFirst("%1", id);
		} else {
			String url = devMode ? SenseUrls.SENSOR_DATA_DEV
	 ...
	 *             In case of communication failure to CommonSense
	 */
	public static JSONObject getUser(Context context) throws IOException,
			JSONException {
		if (null == sAuthPrefs) {
			sAuthPrefs = context.getSharedPreferences(SensePrefs.AUTH_PREFS,
					Context.MODE_PRIVATE);
		...
		String url = devMode ? SenseUrls.CURRENT_USER_DEV
				: SenseUrls.CURRENT_USER;
		...
		if ("200".equalsIgnoreCase(responseCode)) {
			result = new JSONObject(response.get(RESPONSE_CONTENT))
					.getJSONObject("user");
		} else {
	 ...
	 *             In case of communication failure to CommonSense
	 */
	public static boolean joinGroup(Context context, String groupId)
	 ...
	 * @throws IOException
	 *             In case of communication failure to CommonSense
	 * @see SenseServiceStub#changeLogin(String, String,
	 *      nl.sense_os.service.ISenseServiceCallback)
	 */
	 ...
	 */
	public static void registerGCMId(Context context, String registrationId)
			throws IOException, JSONException {
		...
		String pref_registration_id = sAuthPrefs.getString(
				Auth.GCM_REGISTRATION_ID, "");
		...
		String cookie = sAuthPrefs.getString(Auth.LOGIN_COOKIE, null);
		boolean devMode = sMainPrefs.getBoolean(Advanced.DEV_MODE, false);
		String url = devMode ? SenseUrls.REGISTER_GCM_ID_DEV
				: SenseUrls.REGISTER_GCM_ID;
		...
		// check registration result
		JSONObject res = new JSONObject(response.get(RESPONSE_CONTENT));
		if (!registrationId.equals(res.getJSONObject("device").getString(
				GCMReceiver.KEY_GCM_ID))) {
			throw new IllegalStateException(
					"GCM registration_id not match with response");
		}
	 ...
	 *             sensor.
	 */
	public static String registerSensor(Context context, String name,
	 ...
	 * @return 0 if registration completed successfully, -2 if the user already
	 *         exists, and -1 for any other unexpected responses.
	 * @throws JSONException
	 *             In case of unparseable response from CommonSense
	 * @throws IOException
	 *             In case of communication failure to CommonSense
	 * @see SenseServiceStub#register(String, String, String, String, String,
	 *      String, String, String, String,
	 *      nl.sense_os.service.ISenseServiceCallback)
	 */
	public static int registerUser(Context context, String username,
			String password, String name, String surname, String email,
			String mobile) throws JSONException, IOException {
		...
		final String url = devMode ? SenseUrls.REGISTER_DEV
				: SenseUrls.REGISTER;
		...
		if ("201".equalsIgnoreCase(responseCode)) {
			...
			Log.w(TAG, "Failed to register new user! Response code: "
					+ responseCode);
			result = -1;
		}
	 ...
	 * @throws IOException
	 */
	public static Map<String, String> request(Context context,
			String urlString, JSONObject content, String cookie)
			throws IOException {
		...
		try {
			...
			final boolean compress = sMainPrefs.getBoolean(Advanced.COMPRESS,
					true);
	 ...
	 *            Application context, used for getting preferences.
	 * @param email
	 *            Email address for the account that you want to regain access
	 *            to.
	...
	public static boolean resetPassword(Context context, String email)
		...
		// prepare request
		String url = devMode ? SenseUrls.FORGOT_PASSWORD_DEV
				: SenseUrls.FORGOT_PASSWORD;
		...
		if ("200".equals(responseCode)) {
			...
			Log.w(TAG, "Failed to request password reset! Response code "
					+ responseCode);
			return false;
		}
	}
	 ...
	 *             In case of communication failure to CommonSense
	 */
	public static boolean shareSensor(Context context, String sensorId,
		...
		String cookie = sAuthPrefs.getString(Auth.LOGIN_COOKIE, null);
		boolean devMode = sMainPrefs.getBoolean(Advanced.DEV_MODE, false);
		String url = devMode ? SenseUrls.SENSOR_USERS_DEV
				: SenseUrls.SENSOR_USERS;
