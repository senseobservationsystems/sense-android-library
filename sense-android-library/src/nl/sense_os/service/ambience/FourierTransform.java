 ...
 * spectrum.
 * <p>
 * As an example, if you construct a FourierTransform with a <code>timeSize</code> of 1024 and and a
 * <code>sampleRate</code> of 44100 Hz, then the spectrum will contain values for frequencies below
 * 22010 Hz, which is the Nyquist frequency (half the sample rate). If you ask for the value of band
 * number 5, this will correspond to a frequency band centered on
 * <code>5/1024 * 44100 = 0.0048828125 * 44100 = 215 Hz</code>. The width of that frequency band is
 * equal to <code>2/1024</code>, expressed as a fraction of the total bandwidth of the spectrum. The
 * total bandwith of the spectrum is equal to the Nyquist frequency, which in this case is 22100, so
 * the bandwidth is equal to about 50 Hz. It is not necessary for you to remember all of these
 ...
 * <b>Usage</b>
 * <p>
 * A typical usage of a FourierTransform is to analyze a signal so that the frequency spectrum may
 * be represented in some way, typically with vertical lines. You could do this in Processing with
 * the following code, where <code>audio</code> is an AudioSource and <code>fft</code> is an FFT
 * (one of the derived classes of FourierTransform).
 * 
 * <pre>
 ...
 * <b>Windowing</b>
 * <p>
 * Windowing is the process of shaping the audio samples before transforming them to the frequency
 * domain. If you call the <code>window()</code> function with an appropriate constant, such as
 * FourierTransform.HAMMING, the sample buffers passed to the object for analysis will be shaped by
 * the current window before being transformed. The result of using a window is to reduce the noise
 * in the spectrum somewhat.
 * <p>
 * <b>Averages</b>
 * <p>
 * FourierTransform also has functions that allow you to request the creation of an average
 ...
 * <p>
 * <code>linAverages()</code> allows you to specify the number of averages that you want and will
 ...
 * <p>
 * <code>logAverages()</code> will group frequency bands by octave and allows you to specify the
 ...
 * perceive sound.
 * <p>
 * <code>calcAvg()</code> allows you to specify the frequency band you want an average calculated
 ...
 * <p>
 * If you don't want any averages calculated, then you can call <code>noAverages()</code>. This will
 * not impact your ability to use <code>calcAvg()</code>, it will merely prevent the object from
 * calculating an average array every time you use <code>forward()</code>.
 * <p>
 * <b>Inverse Transform</b>
 * <p>
 * FourierTransform also supports taking the inverse transform of a spectrum. This means that a
 * frequency spectrum will be transformed into a time domain signal and placed in a provided sample
 * buffer. The length of the time domain signal will be <code>timeSize()</code> long. The
 * <code>set</code> and <code>scale</code> functions allow you the ability to shape the spectrum
 * already stored in the object before taking the inverse transform. You might use these to filter
...
public abstract class FourierTransform {
    ...
    /**
     * Construct a FourierTransform that will analyze sample buffers that are <code>ts</code>
     * samples long and contain samples with a <code>sr</code> sample rate.
     * 
     * @param ts
    ...
    protected void fillSpectrum() {
        for (int i = 0; i < spectrum.length; i++) {
            spectrum[i] = FloatMath.sqrt(real[i] * real[i] + imag[i] * imag[i]);
        }
     ...
     * linear manner. In other words, each average band will be <code>specSize() / numAvg</code>
     * bands wide.
     * 
     * @param numAvg
    ...
    public void linAverages(int numAvg) {
        if (numAvg > spectrum.length / 2) {
            throw new IllegalArgumentException(
                    "The number of averages for this transform can be at most " + spectrum.length
                            / 2 + ".");
        } else {
     ...
     * rate of 44100 Hz, <code>logAverages(11, 1)</code> will result in 12 averages, each
    ...
    protected void hamming(float[] samples) {
        for (int i = 0; i < samples.length; i++) {
            samples[i] *= (0.54f - 0.46f * FloatMath.cos(TWO_PI * i / (samples.length - 1)));
        }
    }
     ...
     * frequency bands produced by this transform. This is typically equal to
    ...
    /**
     * Sets the amplitude of the <code>i<sup>th</sup></code> frequency band to <code>a</code>. You
     ...
     *            between the starting index and the end of the buffer. If there aren't, an error
    ...
    public void forward(float[] buffer, int startAt) {
        if (buffer.length - startAt < timeSize) {
            throw new IllegalArgumentException(
                    "FourierTransform.forward: not enough samples in the buffer between " + startAt
