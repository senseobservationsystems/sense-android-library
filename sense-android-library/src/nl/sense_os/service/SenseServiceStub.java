...
/**
 * Implementation of the service's AIDL interface. Very closely linked with
 * {@link SenseService}.
 * 
...
public class SenseServiceStub extends Binder {
		...
		new Thread() {
			...
			public void run() {
				...
				try {
					...
					Log.e(TAG,
							"Failed to call back to bound activity after login change: "
									+ e);
				}
			}
		}.start();
	...
	public boolean getPrefBool(String key, boolean defValue) {
		...
		if (key.equals(Status.AMBIENCE) || key.equals(Status.DEV_PROX)
			...
			prefs = service.getSharedPreferences(SensePrefs.STATUS_PREFS,
					Context.MODE_PRIVATE);
		} else {
			prefs = service.getSharedPreferences(SensePrefs.MAIN_PREFS,
					Context.MODE_PRIVATE);
		}
	...
	public long getPrefLong(String key, long defValue) {
		...
		if (key.equals(Auth.SENSOR_LIST_COMPLETE_TIME)) {
			prefs = service.getSharedPreferences(SensePrefs.AUTH_PREFS,
					Context.MODE_PRIVATE);
		} else {
			prefs = service.getSharedPreferences(SensePrefs.MAIN_PREFS,
					Context.MODE_PRIVATE);
	...
	public String getPrefString(String key, String defValue) {
		...
		if (key.equals(Auth.LOGIN_COOKIE) || key.equals(Auth.LOGIN_PASS)
				...
				|| key.equals(Auth.SENSOR_LIST_COMPLETE)
	...
	public void getStatus(ISenseServiceCallback callback)
		...
		callback.statusReport(ServiceStateHelper.getInstance(service)
				.getStatusCode());
	}
	...
	public void register(final String username, final String password,
			final String email, final String address, final String zipCode,
			final String country, final String name, final String surname,
			final String mobile, final ISenseServiceCallback callback) {
		...
		new Thread() {
			...
			@Override
			public void run() {
				int result = service.register(username, password, email,
						address, zipCode, country, name, surname, mobile);
				try {
	...
	public void setPrefBool(String key, final boolean value) {
		...
		if (stored == false) {
			...
			logout();
			// reset GCM id
			SharedPreferences authPrefs = service.getSharedPreferences(
			...
			new Thread() {
				...
				public void run() {
					if (value) {
						Log.w(TAG,
								"USE_COMMONSENSE setting changed: try to log in");
						service.login();
					} else {
						Log.w(TAG,
								"USE_COMMONSENSE setting changed: logging out");
						service.logout();
					}
				}
			}.start();

		} else if (key.equals(Motion.ACCELEROMETER)
				|| key.equals(Motion.LINEAR_ACCELERATION)
				|| key.equals(Motion.GYROSCOPE)
				|| key.equals(Motion.ORIENTATION)
				|| key.equals(Motion.MOTION_ENERGY)
				|| key.equals(Motion.BURSTMODE)
				|| key.equals(Motion.FALL_DETECT)
				|| key.equals(Motion.FALL_DETECT_DEMO)) {
	...
	public void toggleAmbience(boolean active) {
		...
		SharedPreferences prefs = service.getSharedPreferences(
				SensePrefs.STATUS_PREFS, Context.MODE_PRIVATE);
	...
	public void togglePopQuiz(boolean active) {
		Log.w(TAG,
				"Toggle questionnaire ignored: this functionality is no longer supported!");
	}
}
